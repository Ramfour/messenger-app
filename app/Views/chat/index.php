...
<div class="messenger-layout">
    ...
    <aside class="sidebar d-flex flex-column border-end bg-white">
        <div class="sidebar-top px-3 py-2 border-bottom d-flex align-items-center justify-content-between">
            ...
            <a href="/contacts" class="text-secondary" title="Контакты">&#128100;</a>
        </div>
        ...
        <div class="px-2 py-2 border-bottom position-relative">
            <input type="text" id="user-search" class="form-control form-control-sm rounded-pill"
                   placeholder="Поиск пользователей..." autocomplete="off">
            <ul id="search-results" class="search-dropdown list-unstyled d-none"></ul>
        </div>

        <div class="px-3 pt-2 pb-1">
            <span class="text-uppercase text-muted" style="font-size:10px;letter-spacing:.06em">Личные</span>
        </div>
        ...
        <div class="px-3 pt-2 pb-1 border-top">
            <span class="text-uppercase text-muted" style="font-size:10px;letter-spacing:.06em">Группы</span>
        </div>
    ...
    <main class="chat-main d-flex flex-column">
        <?php echo $chatContent ?? '<div class="flex-grow-1 d-flex align-items-center justify-content-center text-muted">Выберите чат для начала общения</div>'; ?>
    </main>

    <!-- Right nav -->
    <nav class="right-nav d-flex flex-column border-start bg-white p-3 gap-3">
        <a href="/profile" class="text-secondary small">Профиль</a>
        <a href="/groups/create" class="text-secondary small">Создать группу</a>
        <a href="/logout" class="text-secondary small">Выйти</a>
    </nav>
